...
def load_pdf_page_as_byte(file_name, page_number):
    ...
    pages = pdf2image.convert_from_path(
        ...
        dpi=300,
        poppler_path="./poppler-21.03.0/Library/bin",
    )
...
def get_response(file_name):
    ...
    client = boto3.client(
        "textract",
        aws_access_key_id="AKIA6MZEBQT46D2DWPWU",
        aws_secret_access_key="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",
...
def get_expense_table(response):
    df = pd.DataFrame()
    for line_item in response.get("ExpenseDocuments")[0]["LineItemGroups"][0].get(
        "LineItems"
    ):
        d_line_item = {}
